...
export default {
  ...
  methods: {
    async createHotel () {
      ...
      await axios.post(`http://localhost:8080/api/hotels/?name=${this.name}&description=${this.description}&address=${this.address}&phone=${this.phone}`, data, {
        headers: {
    ...
    async payEmployee (id, name, surname, mobilePhone, payPerHour) {
      ...
      await axios.put(`http://localhost:8080/api/employees/${id}?name=${name}&surname=${surname}&mobilePhone=${mobilePhone}&payPerHour=${payPerHour}&hoursWorked=0`, null, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      })
